...
export function RestaurantMenuHighlights({ section }: { section: MenuHighlightsProps }) {
  const { badgeIcon, badgeText, title, titleHighlight, description, items = [], buttonText, buttonIcon, buttonUrl } = section
  ...
  return (
    <section className="r-section r-menu-highlights" id="menu-highlights">
      <div className="r-container">
        ...
        {buttonText && (
          <div className="r-center-btn"><a href={buttonUrl || '#menu'} className="r-btn r-btn-primary">{buttonText} {buttonIcon && <i className={buttonIcon}></i>}</a></div>
        )}
      </div>
    </section>
  )
