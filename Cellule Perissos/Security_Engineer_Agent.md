...
## 🏗️ Sprint 2: Security Hardening

### Stripe Webhook Security

```typescript
// Verify Stripe webhook signatures to prevent replay attacks
...
// 1. Short-lived (15 minutes max)
// 2. Single-use (consumed on first request)
// 3. Scoped to specific content (collection + slug)
...
// Generate token (in Back Office API route)
const token = await new SignJWT({
  ...
  .setIssuedAt()
  .setExpirationTime('15m')  // 15 minutes max
  .sign(SECRET)

// Verify token (in Frontend middleware)
...
```nginx
# Nginx / Vercel vercel.json headers
{
  "headers": [
    {
      "source": "/(.*)",
      "headers": [
        ...
        { "key": "Permissions-Policy", "value": "camera=(), microphone=(), geolocation=()" },
        {
          ...
          "value": "default-src 'self'; script-src 'self' 'unsafe-inline' 'unsafe-eval'; style-src 'self' 'unsafe-inline'; img-src 'self' data: https://media.perissos.dev; font-src 'self'; connect-src 'self' https://api.stripe.com; frame-ancestors 'none'; base-uri 'self'; form-action 'self';"
        },
        {
...
### Security Audit Checklist — Sprint 2

```markdown
## Security Audit Checklist

### Authentication & Authorization
- [ ] JWT tokens have appropriate expiration (15min preview, 7d session)
...
- [ ] RBAC enforced on all collections (admin, editor, viewer)

### Stripe Integration
- [ ] Webhook signatures verified on all endpoints
- [ ] Stripe secret key never exposed to client-side
- [ ] Customer IDs stored securely (not in localStorage)
- [ ] Payment intent amounts validated server-side
- [ ] Idempotency keys used for payment operations

### Preview System
- [ ] Preview tokens expire within 15 minutes
- [ ] Tokens are single-use (consumed on first request)
- [ ] Tokens are scoped to specific content
- [ ] Draft content not cached by CDN
- [ ] Preview route not indexed by search engines

### API Security
- [ ] Rate limiting active on all API routes (100 req/60s per IP)
- [ ] Input validation on all endpoints (Zod or similar)
- [ ] SQL injection prevention (parameterized queries via Payload)
- [ ] CORS configured restrictively (only client domains)
- [ ] Request body size limits enforced

### Data Protection
- [ ] Database connections encrypted (TLS)
- [ ] Media files served via HTTPS (R2 public URL)
...
- [ ] Neon database access restricted to Vercel IPs
- [ ] R2 bucket CORS configured for client domains only
- [ ] Sentry DSN does not expose sensitive information
- [ ] All third-party dependencies audited (npm audit)
```
